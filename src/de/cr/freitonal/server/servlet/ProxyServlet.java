...
public class ProxyServlet extends HttpServlet {
		...
		if (intProxyResponseCode >= HttpServletResponse.SC_MULTIPLE_CHOICES /* 300 */
			...
			if (followRedirects) {
				if (stringLocation.contains("jsessionid")) {
					Cookie cookie = new Cookie("JSESSIONID", stringLocation.substring(stringLocation
	...
	private void setProxyRequestCookies(HttpServletRequest httpServletRequest, HttpMethod httpMethodProxyRequest) {
		...
		for (Cookie cookie : cookies) {
			...
			httpMethodProxyRequest.setRequestHeader("Cookie", cookie.getName() + "=" + cookie.getValue() + "; Path="
					+ cookie.getPath());
		}
	}
